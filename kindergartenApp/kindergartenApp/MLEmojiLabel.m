...
+ (CGSize)boundingRectWithSize:(NSString *)text w:(CGFloat)width font:(CGFloat)fontSize {
    ...
    _emojiLabel.font = [UIFont systemFontOfSize:fontSize];
    ...
    NSInteger page = (_emojiLabel.frame.size.width + width - 1) / width;
    
    NSLog(@"f:%@,b:%@", NSStringFromCGRect(_emojiLabel.frame), NSStringFromCGRect(_emojiLabel.bounds));
    return CGSizeMake(width, page * 20);
}
